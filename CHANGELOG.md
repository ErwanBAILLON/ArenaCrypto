...
### Added
- Live exits: `arena live` (Deployment `arena-live`) polls perp prices every 5 s and executes stops and ROI take-profits between ticks -- ladder families on their excess-over-basket measure, any competitor given `live_stop` / `live_roi` on the leg's raw return. Fills are written at the streamed price, the book is re-stated, the competitor forgets the leg (with a cooldown), Telegram is told, and the next tick accounts the leg to its fill price. Quiet windows around tick minutes. Spec: `docs/specs/2026-09-24-live-exits.md`.
