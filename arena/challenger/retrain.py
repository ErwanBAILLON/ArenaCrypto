...
def build_dataset(
    meta: MetaLabel, history: HistoryFrames, symbols: list[str], start: datetime, end: datetime, step_hours: int = 4
) -> pd.DataFrame:
    """Replay the bases over ``[start, end]`` every ``step_hours`` and label each signal."""
    full = Snapshot.from_long(
        end, symbols, history.candles, history.funding, history.open_interest, None, history.news, history.macro_events
    )
    closes = full.closes()
    bars = closes.index[
        (closes.index >= pd.Timestamp(start)) & (closes.index <= pd.Timestamp(end) - pd.Timedelta(hours=HORIZON))
    ]
...
def run(
    ...
    trial_id = registry.add_trial(
        conn, "meta_label", "retrain", {"bases": base_params, "window_days": window_days}, {"rows": len(df)}, None
    )
    conn.commit()
    ...
    if not np.isfinite(auc) or auc < min_auc:
        ...
        bstore.add_alert(
            conn,
            Alert(kind="rejected", payload={"detail": f"meta_label retrain rejected: wf AUC {auc:.3f} < {min_auc}"}),
        )
        conn.commit()
        return None
    ...
    spec = CompetitorSpec(
        None,
        f"meta_label_v{version}",
        "meta_label",
        version,
        {"bases": base_params, "threshold": 0.55},
        status="challenger",
