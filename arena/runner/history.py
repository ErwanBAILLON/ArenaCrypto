...
def load_history(conn: psycopg.Connection, universe: Universe, start: datetime, end: datetime, with_news: bool = True) -> HistoryFrames:
    ...
    news = nstore.news_features(conn, syms, max(start, end - timedelta(days=30)), end, SCORER_VERSION) if with_news else None
...
def snapshot_from_history(h: HistoryFrames, ts: datetime, symbols: list[str], hl_funding: dict[str, float] | None = None) -> Snapshot:
