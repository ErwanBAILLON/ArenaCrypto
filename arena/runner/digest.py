...
def build(conn: psycopg.Connection, now: datetime) -> str:
    ...
    for s in specs:
        ...
        rows.append({
            "name": s.name, "status": s.status, "role": s.role,
    ...
    alloc = [(names.get(cid, str(cid)), w) for cid, w in sorted(bstore.last_allocations(conn).items(), key=lambda kv: -kv[1])]
    ...
    for s in specs:
        ...
        challengers.append({"name": s.name, "days_in_arena": days, "decisions": n,
                            "progress": f"{days}/{promotion.MIN_DAYS}d, {n}/{promotion.MIN_DECISIONS} dec"})
    with conn.cursor() as cur:
        cur.execute("SELECT kind, payload FROM alerts WHERE ts >= %s AND kind IN ('drift','stale','error','rejected') ORDER BY ts", (now - timedelta(days=1),))
